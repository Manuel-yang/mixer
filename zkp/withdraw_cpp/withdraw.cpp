...
uint get_total_signal_no() {return 59034;}

uint get_number_of_components() {return 2342;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 28300;}
...
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[44]); // line circom 21
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 1;
{
...
PFrElement aux_dest = &lvar[0];
// load src
Fr_add(&expaux[0],&lvar[0],&circuitConstants[1]); // line circom 21
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[0],&circuitConstants[44]); // line circom 21
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[74]); // line circom 39
while(Fr_isTrue(&expaux[0])){
...
Fr_eq(&expaux[0],&signalValues[mySignalStart + 1],&ctx->signalValues[ctx->componentMemory[mySubcomponents[39]].signalStart + 0]); // line circom 50
...
uint csoffset = mySignalStart+23431;
for (uint i = 0; i < 1; i++) {
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 1],&signalValues[mySignalStart + 1]); // line circom 47
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[74]); // line circom 52
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 1;
{
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 2],&signalValues[mySignalStart + 2]); // line circom 65
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 4],&signalValues[mySignalStart + 4]); // line circom 66
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + 3]); // line circom 67
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 5],&signalValues[mySignalStart + 5]); // line circom 68
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 2; i++){
